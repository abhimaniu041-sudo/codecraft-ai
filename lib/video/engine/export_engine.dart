...
enum ExportStatus { idle, preparing, rendering, done, failed }
...
class ExportEngine {
  final StreamController<ExportProgress> _ctrl =
      StreamController<ExportProgress>.broadcast();

  Stream<ExportProgress> get progressStream => _ctrl.stream;
  ...
  Future<String?> exportToHTML(
    ...
    try {
      ...
      final html = _buildHTML(scenes, projectName);
      ...
      final filename = '${projectName.replaceAll(' ', '_')}_$ts.html';
  ...
  String _buildHTML(List<StoryScene> scenes, String projectName) {
...
.pbtn{background:linear-gradient(135deg,#6c63ff,#3ecfcf);border:none;padding:11px 28px;border-radius:20px;cursor:pointer;color:#fff;font-size:18px}
...
function drawVolcano(){CX.fillStyle='#3D0000';CX.fillRect(0,H*.63,W,H*.37);CX.fillStyle='#4A0000';CX.beginPath();CX.moveTo(W*.35,H*.63);CX.lineTo(W*.5,H*.22);CX.lineTo(W*.65,H*.63);CX.fill();CX.fillStyle='rgba(255,69,0,.8)';CX.beginPath();CX.arc(W*.5,H*.25,20,0,Math.PI*2);CX.fill();}
function drawCastle(){CX.fillStyle='#3A3A3A';CX.fillRect(0,H*.63,W,H*.37);CX.fillStyle='#585858';CX.fillRect(W*.28,H*.2,W*.44,H*.43);CX.fillStyle='#4A4A4A';CX.fillRect(W*.13,H*.33,W*.16,H*.3);CX.fillRect(W*.71,H*.33,W*.16,H*.3);}
...
function drawSnow(){CX.fillStyle='#fff';CX.fillRect(0,H*.63,W,H*.37);}
...
function drawHumanB(pal,sz,st,type,ll,rl,la,ra,mo){const bw=sz*.38,bh=sz*.32,lw=sz*.11,lh=sz*.22,aw=sz*.10,ah=sz*.24,hr=sz*.22;[[-1,ll],[1,rl]].forEach(([s,a])=>{CX.save();CX.translate(s*bw*.28,bh*.5);CX.rotate(a);const lg=CX.createLinearGradient(0,0,0,lh);lg.addColorStop(0,pal.body);lg.addColorStop(1,pal.body+'BB');CX.fillStyle=lg;rr(-lw/2,0,lw,lh,lw/2);CX.fill();CX.fillStyle='#222';rr(-lw*.8,lh-lw*.4,lw*1.8,lw*.7,lw*.4);CX.fill();CX.restore();});const bg=CX.createLinearGradient(-bw*.5,-bh*.5,bw*.5,bh*.5);bg.addColorStop(0,lighter(pal.body,40));bg.addColorStop(1,pal.body);CX.fillStyle=bg;rr(-bw/2,-bh/2,bw,bh,bw*.22);CX.fill();if(type==='hero'){CX.fillStyle=pal.acc+'CC';CX.beginPath();CX.moveTo(-bw*.44,-bh*.3);CX.quadraticCurveTo(-bw*.85,bh*.4,-bw*.3,bh*.55);CX.lineTo(-bw*.44,-bh*.3);CX.fill();drawStar5(0,-bh*.05,bw*.1,pal.acc);}else if(type==='villain'){CX.fillStyle='#1A0000CC';CX.beginPath();CX.moveTo(-bw*.5,-bh*.35);CX.quadraticCurveTo(-bw*.95,bh*.5,-bw*.2,bh*.55);CX.lineTo(-bw*.5,-bh*.35);CX.fill();}[[-1,la],[1,ra]].forEach(([s,a])=>{CX.save();CX.translate(s*bw*.52,-bh*.1);CX.rotate(a);const ag=CX.createLinearGradient(0,0,0,ah);ag.addColorStop(0,pal.body);ag.addColorStop(1,pal.skin);CX.fillStyle=ag;rr(-aw/2,0,aw,ah,aw/2);CX.fill();CX.fillStyle=pal.skin;CX.beginPath();CX.arc(0,ah+aw*.15,aw*.48,0,Math.PI*2);CX.fill();CX.restore();});CX.fillStyle=pal.skin;rr(-bw*.1,-bh*.52,bw*.2,bh*.12,4);CX.fill();const hy=-bh*.88;CX.fillStyle=pal.skin;CX.beginPath();CX.ellipse(0,hy,hr,hr*.88,0,0,Math.PI*2);CX.fill();CX.fillStyle=pal.hair;CX.beginPath();CX.ellipse(0,hy,hr*1.02,hr*.88,0,-Math.PI,0);CX.fill();[[-hr*.35,hy-hr*.05],[hr*.35,hy-hr*.05]].forEach(([ex,ey])=>{CX.fillStyle='white';CX.beginPath();CX.ellipse(ex,ey,hr*.16,hr*.14,0,0,Math.PI*2);CX.fill();CX.fillStyle='#111';CX.beginPath();CX.arc(ex,ey+hr*.02,hr*.055,0,Math.PI*2);CX.fill();CX.fillStyle='white';CX.beginPath();CX.arc(ex-hr*.04,ey-hr*.04,hr*.025,0,Math.PI*2);CX.fill();});const my=hy+hr*.32;CX.strokeStyle='#333';CX.lineWidth=2;CX.lineCap='round';if(st==='happy'||st==='victory'){CX.beginPath();CX.moveTo(-hr*.28,my-hr*.04);CX.quadraticCurveTo(0,my+hr*.24,hr*.28,my-hr*.04);CX.stroke();}else if(st==='sad'){CX.beginPath();CX.moveTo(-hr*.24,my+hr*.06);CX.quadraticCurveTo(0,my-hr*.12,hr*.24,my+hr*.06);CX.stroke();}else if(st==='angry'){CX.beginPath();CX.moveTo(-hr*.26,my+hr*.02);CX.lineTo(hr*.26,my+hr*.02);CX.stroke();}else if(st==='talk'){const oa=mo*hr*.15+hr*.04;CX.fillStyle='#880E4F';CX.beginPath();CX.ellipse(0,my,hr*.15,oa,0,0,Math.PI*2);CX.fill();}else{CX.beginPath();CX.moveTo(-hr*.18,my);CX.lineTo(hr*.18,my);CX.stroke();}}
